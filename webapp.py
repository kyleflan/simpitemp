...
def return_temp_hum():
    ...
    df_hum = pd.DataFrame(tlrd_hum.d.values(), index=tlrd_hum.d.keys(), columns=['hum'])
    df_hum.sort_index(inplace=True)
    df_temp = pd.DataFrame(tlrd_temp.d.values(), index=tlrd_temp.d.keys(), columns=['temp'])
    df_temp.sort_index(inplace=True)

    removed_outliers_hum = df_hum['hum'].between(df_hum['hum'].quantile(.01), df_hum['hum'].quantile(0.99))
    removed_outliers_temp = df_temp['temp'].between(df_temp['temp'].quantile(.01), df_temp['temp'].quantile(0.99))
    ...
    temp_labels = list(df_temp.index)
    hum_labels = list(df_hum.index)
    ...
    formatted_temp_labels = [datetime.datetime.strptime(label, '%Y-%m-%dT%H%M%S.%f').strftime('%H:%M:%S') for label in temp_labels]

    # get "instantaneous" metrics, which are the values at the latest index
    temp_inst = df_temp.loc[df_temp.idxmax()].squeeze()
    hum_inst = df_hum.loc[df_hum.idxmax()].squeeze()
